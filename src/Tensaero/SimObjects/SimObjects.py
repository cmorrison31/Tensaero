...
class BaseObject(ABC):
    ...
    @abstractmethod
    def init_state(self, time: JulianDate.JulianDate, position: State.Position,
    ...
    def _velocity_to_inertial_frame(self, jd_utc: JulianDate.JulianDate,
        ...
        if (velocity.reference_frame ==
                State.ReferenceFrames.EarthCenteredEarthFixed):
            v_bi_i = velocity + omega_ei_i @ position
...
class FixedGroundPoint(BaseObject):
    ...
    def init_state(self, time: JulianDate.JulianDate, position: State.Position,
        ...
        # Heading and flight path angle will always be 0, 0 for a fixed
        # ground point
        chi = heading_angle
